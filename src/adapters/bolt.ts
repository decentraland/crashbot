...
export async function createBoltComponent(
...
): Promise<BoltComponent> {
  const { pg, config } = components

  const userToken = await config.requireString('SLACK_USER_TOKEN')
  const botToken = await config.requireString('SLACK_BOT_TOKEN')
  ...
  app.view('create', async ({ ack, body, view, client, logger }) => {
    ...
    try {
      ...
      const reportDateTimeObj = values['report_date_time'].report_date_time as {
        type: string
      ...
      const queryResult = await pg.query(
        CREATE_INCIDENT(user, severity, title, description, point, contact, reportedAt)
      )
  ...
  app.view('update', async ({ ack, body, view, client, logger }) => {
    ...
    try {
      ...
      const reportTimestamp = reportDateTimeObj.selected_date_time * 1000
      const resolutionDateTimeObj = values['resolution_date_time'].resolution_date_time as {
        type: string
        selected_date_time: number
      }
      ...
      if (closedAt) msg += `*Resolution date and time:* ${closedAt}hs\n`
      ...
      if (rcaLink) msg += `*RCA link:* ${rcaLink}`
...
function getIncidentView(options: IncidentViewOptions): View {
  ...
  if (options.callbackId === 'update') {
    ...
    if (options.status) {
      ...
      if (options.status === 'closed') resolutionDateTimeBlock.optional = false
    }
    ...
    if (options.rcaLink) rcaLinkBlock.element.initial_value = options.rcaLink
